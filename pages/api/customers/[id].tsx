...
export const editCustomer = async(id: string | ObjectId, customer: Customer) => {
    id = typeof id === 'string' ? new ObjectId(id) : id
    const mongoClient = await clientPromise

    const response = await mongoClient.db().collection('customers').replaceOne({_id: id}, customer)
    return response
}
...
export default async (req: NextApiRequest, res: NextApiResponse<{modifiedCount: number} | {customer: Customer} | {error:string} | {deletedCount: number}>) => {
